//
//	SetPaperles.swift
//	Model file Generated using: 
//	Vin.Favara's JSONExportV https://github.com/vivi7/JSONExport 
//	(forked from Ahmed-Ali's JSONExport)
//

import Foundation
import ObjectMapper

/// Clase que representa los datos del Request utilizado para consumir el servicio SetPaperless. Aplica la selección del usuario en correspondencia a la opción de paperless
/// Forma parte del adaptador MC_BillingManagementAdapter
class SetPaperles : BaseRequest{

    var accountId : String?
    var lineOfBusiness : String?
    var userEmailforPaperless : String?
    var userMobileNumberforPaperless : String?
    var userProfileID : String?
    var isTermsAndConditionsAccepted : Bool?

    override init() {
        super.init()
    }

    required init?(map: Map) {
        super.init(map: map);
    }

    override func mapping(map: Map)
    {
        super.mapping(map: map);
		accountId <- map["AccountId"]
		lineOfBusiness <- map["LineOfBusiness"]
		userEmailforPaperless <- map["UserEmailforPaperless"]
		userMobileNumberforPaperless <- map["UserMobileNumberforPaperless"]
		isTermsAndConditionsAccepted <- map["isTermsAndConditionsAccepted"]
		
	}

}
