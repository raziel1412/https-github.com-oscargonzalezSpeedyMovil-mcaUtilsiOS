//
//  CardsTodoClaroManagement.swift
//  MiClaro
//
...
class CardsTodoClaroManagement: UIViewController {
    ...
    override func viewWillDisappear(_ animated: Bool) {
        print("SE SALIO DE LA VISTA DE CARDS TODO CLARO")
        for viewTmp in self.view.subviews {
            viewTmp.removeFromSuperview()
        }
    }
    
    init(frame: CGRect, services: [CardsTodoClaroModel]) {
        super.init(nibName: nil, bundle: nil)
        self.view.frame = frame
        self.arrayTodoClaroData = services
    }
    ...
    private func createInterface() {
        ...
        self.viewContentGraphic = CardContentDetailGraphic(frame: frameContentGraphic, typeAccount: .TodoClaro)
    ...
    func updateDataServiceSelected() {
        if self.arrayTodoClaroData.count > 0 {
            let tvData = self.arrayTodoClaroData[indexAccountSelected]
            ...
            self.viewHeaderPlan.updateDataHeader(typeCard: .TodoClaro, typeService: .TodoClaro, namePlan: tvData.planDetail.namePlan)
            
            self.viewBalance.setDataBalance(balance: tvData.planDetail.balancePlan, dateVigency: tvData.planDetail.datePlanVigency, typeAccount: .TodoClaro)
            ...
            AnalyticsInteractionSingleton.sharedInstance.ADBTrackViewServicios(viewName: "Mis servicios|Todo Claro", type: "2", nombreServicio: "Todo Claro", detalleServicio: tvData.planDetail.namePlan, fechaVencimiento: (tvData.planDetail.datePlanVigency), estatus: "", saldo: strSaldo)
        }
    }
    ...
    func refreshInfo(refresh: UIRefreshControl) {
        if SessionSingleton.sharedInstance.shouldRefresh() {
            ...
            print("REFRESH SERVICES CARDS TODO CLARO")
...
extension CardsTodoClaroManagement: CardBalanceViewDelegate {
...
extension CardsTodoClaroManagement: CardButtonBoletaViewDelegate {
