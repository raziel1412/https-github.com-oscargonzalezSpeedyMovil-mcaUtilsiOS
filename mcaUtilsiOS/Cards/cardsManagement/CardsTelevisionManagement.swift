//
//  CardsTelevisionManagement.swift
//  MiClaro
//
...
class CardsTelevisionManagement: UIViewController {

    /*********************** Variables ***********************/
    var arrayTelevisionData: [CardsTelevisionModel] = []
    var televisionList: [OptionModel]! = [OptionModel]()
    ...
    init(frame: CGRect, services: [CardsTelevisionModel]) {
        super.init(nibName: nil, bundle: nil)
        self.view.frame = frame
        self.arrayTelevisionData = services
    }
    ...
    private func createInterface() {
        ...
        if arrayTelevisionData.count < 2 {
        ...
        self.viewContentGraphic = CardContentDetailGraphic(frame: frameContentGraphic, typeAccount: .Television)
        self.scrollContent.addSubview(self.viewContentGraphic)
        self.viewContentGraphic.hiddeButtonSelection()
        
        self.televisionList.removeAll()
        for tv in arrayTelevisionData {
            let aModel = OptionModel(isSelected: false, detailText: tv.addressHome)
            televisionList.append(aModel)
        }
    }
    
    func updateDataServiceSelected() {
        if self.arrayTelevisionData.count > 0 {
            let tvData = self.arrayTelevisionData[indexAccountSelected]
            ...
            self.viewHeaderPlan.updateDataHeader(typeCard: .Televisión, typeService: .Television, namePlan: tvData.planDetail.namePlan)
            
            self.viewBalance.setDataBalance(balance: tvData.planDetail.balancePlan, dateVigency: tvData.planDetail.datePlanVigency, typeAccount: .Television)
            ...
            AnalyticsInteractionSingleton.sharedInstance.ADBTrackViewServicios(viewName: "Mis servicios|Television", type: "1", nombreServicio: "Television", detalleServicio: tvData.planDetail.namePlan, fechaVencimiento: (tvData.planDetail.datePlanVigency), estatus: "", saldo: strSaldo)
        }
    }
    ...
    func showTicketBoleta() {
        ...
        vcShowTicket.selectedTypeService = "4"/*selectedServiceType*/
...
/*func retrieveChannelsOffer(){
    let req = RetrieveChannelsOfferRequest()
    req.retrieveChannelsOffer?.userProfileID = "16268505-8"
    req.retrieveChannelsOffer?.lineOfBusiness = "1"
    req.retrieveChannelsOffer?.accountId = "10196942"
    
    WebServicesWithObjects.executeRetrieveChannelsOffer(params: req, onSuccess: { (result) in
        print(result)
...
func updateChannelStatus(){
    let req = UpdateChannelStatusRequest()
    
    
    WebServicesWithObjects.executeUpdateChannelStatus(params: req,
                                                      onSuccess: { (result) in
                                                        print(result);
...
extension CardsTelevisionManagement: CardBalanceViewDelegate {
    func btnBalanceAction(webId: String) {
        AnalyticsInteractionSingleton.sharedInstance.ADBTrackCustomLink(viewName: "Mis servicios|Television:Pagar boleta")
...
extension CardsTelevisionManagement: CardButtonBoletaViewDelegate {
    func actionActiveBoleta() {
        self.showTicketBoleta()
        AnalyticsInteractionSingleton.sharedInstance.ADBTrackCustomLink(viewName: "Mis servicios|Television:Activar boleta electronica")
    }
    
    func actionSendBoleta() {
        self.showTicketBoleta()
        AnalyticsInteractionSingleton.sharedInstance.ADBTrackCustomLink(viewName: "Mis servicios|Television:Enviar boleta email")
    }
    
    func actionDownloadBoleta() {
        self.showTicketBoleta()
        AnalyticsInteractionSingleton.sharedInstance.ADBTrackCustomLink(viewName: "Mis servicios|Television:Descargar boleta")
    }
}
//MARK: CardContentDetailDelegate
extension CardsTelevisionManagement: CardContentDetailDelegate {
    func consumingOptionSelected() {
        AnalyticsInteractionSingleton.sharedInstance.ADBTrackCustomLink(viewName: "Mis servicios|Television:Consumo")
    }
    func detailPlanSelected() {
        AnalyticsInteractionSingleton.sharedInstance.ADBTrackCustomLink(viewName: "Mis servicios|Television:Detalle de plan")
    }
...
// MARK: selected service view
extension CardsTelevisionManagement: CardSelectServiceDelegate {
    
    func showPopupServices() {
        ListOptionsVC.show(inViewController: self, array: televisionList, title: textConfiguration?.translations?.data?.generales?.tv ?? "Televisión", indexSelected: indexAccountSelected)
    }
    
    override func indexSelected(selected:Int) {
        self.indexAccountSelected = selected
        self.updateDataServiceSelected()
    }
}
